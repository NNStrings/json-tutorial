...
static int lept_parse_string_raw(lept_context* c, char** s, size_t* len) {
    ...
    for (;;) {
        ...
        switch (ch) {
            case '\"':
                *len = c->top - head;
                *s = (const char*)lept_context_pop(c, *len);
                c->json = p;
                return LEPT_PARSE_OK;
            case '\\':
                switch (*p++) {
...
static int lept_parse_object(lept_context* c, lept_value* v) {
    ...
    for (;;) {
        ...
        if (m.v.type == LEPT_STRING) {
            m.klen = lept_get_string_length(&m.v);
            m.k = lept_get_string(&m.v);
        }
        else {
            ret = LEPT_PARSE_MISS_KEY;
            break;
        ...
        if ((ret = lept_parse_value(c, &m2.v)) != LEPT_PARSE_OK) {
            ret = LEPT_PARSE_MISS_COMMA_OR_CURLY_BRACKET;
            break;
        }
        memcpy(&m.v, &m2.v, sizeof(lept_value));
        memcpy(lept_context_push(c, sizeof(lept_member)), &m, sizeof(lept_member));
        size++;
        m.k = NULL;
        m2.k = NULL; /* ownership is transferred to member on stack */
